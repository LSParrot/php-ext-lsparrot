#ifndef LSP_PHP84_ABSTRACTION_H
# define LSP_PHP84_ABSTRACTION_H

# include "lsp_internal.h"

static inline const char *lsp_php84_ast_kind_name(zend_ast_kind kind)
{
	switch (kind) {
		...
		case ZEND_AST_CLONE: return "CLONE";
		case ZEND_AST_EXIT: return "EXIT";
...
static inline bool lsp_php84_ast_is_decl(zend_ast *ast)
{
	return zend_ast_is_special(ast) && ast->kind >= ZEND_AST_FUNC_DECL;
}

static inline bool lsp_php84_ast_is_opaque_node(zend_ast_kind kind)
{
	return kind == ZEND_AST_ZNODE;
}
